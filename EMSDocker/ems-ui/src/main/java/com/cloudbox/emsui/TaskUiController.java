...
public class TaskUiController {
    ...
    String getAllTasks(Model model){
        ...
        try {
            ResponseEntity<Task[]> tasklist = restTemplate.exchange("http://dockertask:8383/tasks", HttpMethod.GET,gethttpEntity(),Task[].class);
    ...
    String deletetask(@PathVariable Integer id, Model model){
        ...
        try{
            restTemplate.exchange("http://dockertask:8383/tasks/"+id,HttpMethod.DELETE,gethttpEntity(),Task.class);
            status = "del_active";
        }catch (HttpStatusCodeException ex){
            status ="del_error";
    ...
    String saveTask(@ModelAttribute("task") Task task){
        ...
        try{
            ResponseEntity<Task> savetask = restTemplate.postForEntity("http://dockertask:8383/tasks",httpEntity,Task.class);
            status="active";
        }catch (HttpStatusCodeException ex){
            Logger logger = Logger.getLogger(TaskUiController.class.getName());
            logger.warning(ex.toString());
